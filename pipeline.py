...
def call_gemini(app_details, reviews):
    ...
    reviews_serializable = [{k: make_serializable(
        v) for k, v in review.items()} for review in reviews]
...
"""
    ...
    try:
        ...
        print(
            f"❌ Error: Invalid JSON response from Gemini: {cleaned_response}")
        return {"type": "genuine", "reason": "Could not parse response, assuming app is legitimate."}
